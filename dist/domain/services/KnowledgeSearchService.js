...
export class KnowledgeSearchService {
    ...
    findById(allKnowledge, id) {
        ...
        // タイトルのハッシュで検索
        const item = items.find(item => this.hashTitle(item.getTitle()) === titleHash.trim());
        if (!item) {
            return null;
        }
     ...
     * ID形式: `{category}/{language}/{titleHash}`
     */
    generateId(category, language, item) {
