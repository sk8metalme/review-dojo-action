 ...
 */
function createKnowledgeRepository(mode, serializer) {
    if (mode.type === 'github') {
...
class ReviewDojoMcpServer {
     ...
     */
    async handleGetKnowledgeDetail(args) {
     ...
     */
    async handleGeneratePRChecklist(args) {
        ...
        // 各要素が文字列であることを確認
        if (!args.filePaths.every((path) => typeof path === 'string' && path.trim().length > 0)) {
